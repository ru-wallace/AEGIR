...
void ms5837_i2c_read( ms5837_t *sensor, uint8_t command, uint8_t *data, uint8_t num_bytes )
{
    ...
    {
        sensor->user_read_fn(sensor->pigpiod_instance_handle, sensor->i2c_bus, MS5837_ADDR, command, data, num_bytes );
    }
}
...
void ms5837_i2c_write( ms5837_t *sensor, uint8_t command )
{
    ...
    {
        sensor->user_write_fn(sensor->pigpiod_instance_handle, sensor->i2c_bus, MS5837_ADDR, command, 0, 0 );
    }
}
...
bool ms5837_calculate( ms5837_t *sensor )
{
    ...
    int64_t pressure_offset = ((int64_t)sensor->calibration_data[C2_PRESSURE_OFFSET] * ((uint32_t)1 << 16)) 
                              + ((int64_t)sensor->calibration_data[C4_TEMP_PRESSURE_OFFSET_COEFF] * delta_temp)/(1 << 7);

    // Pressure Sensitivity at actual temp - 41-bit signed
    // SENS = SENS T1 + TCS * dT 
    //      = C1 * 2^16 + (C3 * dT ) / 2^7
    int64_t pressure_sensitivity = (int64_t)sensor->calibration_data[C1_PRESSURE_SENSITIVITY] * ((uint32_t)1 << 15) 
                                   + ( (int64_t)sensor->calibration_data[C3_TEMP_PRESSURE_SENSITIVITY_COEFF] * delta_temp)/((uint32_t)1 << 8); 
    ...
    if( (temperature / 100U) < 20 )
    {
        // Ti = 11 * dT^2 / 2^35
        // Ti = 3*dT^2 / (1 << 33)
         temp_i = ( 3 * (int64_t)delta_temp*(int64_t)delta_temp ) / ((uint64_t)1 << 33);
        
        // OFFi = 31 * (TEMP - 2000)^2 / 2^3
        // OFFi = 3* (temperature-2000)*(temperature-2000) / (1 << 1);
         offset_i = ( 3 * (temperature-2000)*(temperature-2000) ) / (1 << 1);

        // SENSi = 63 * (TEMP - 2000)^2 / 2^5
        // SENSi = (5* (temperature-2000)*(temperature-2000) ) / (1 << 3);
         sensitivity_i = ( 5 * (temperature-2000)*(temperature-2000) ) / (1 << 3);

         if (temperature / 100 < -15)
         {
            // Additional compensation for very low temperatures
            // OFFi = OFFi + 7 * (TEMP + 1500)^2
            offset_i += (7 * (temperature + 1500) * (temperature + 1500));

            //SENSi = SENSi + 4 * (TEMP + 1500)^2
            sensitivity_i += (4 * (temperature + 1500) * (temperature + 1500));

            
         } else if (temperature / 100 >= 20)
         {
            temp_i = 2 * (int64_t)delta_temp*(int64_t)delta_temp / ((uint64_t)1 << 37);

            offset_i = ((temperature - 2000) * (temperature - 2000)) / (1 << 4);
    ...
    int32_t pressure_2 = ( ( (sample_pressure * sensitivity_2) / ((uint32_t)1 << 21) - offset_2 ) / ((uint32_t)1 << 13) ) * 10;
